...
def load_normal_grained_refs(ref_folder: pathlib.Path, squash_num: int, commits: list[str],
                             coarse_grained_commit: str) -> list[list[Refactoring]]:
    refs = []
    fine_grained_path = ref_folder.joinpath(f"o{squash_num}").joinpath(coarse_grained_commit)
    for commit in commits:
        refs.append(load_dictref(fine_grained_path.joinpath(commit + ".json")))
    return refs


def load_coarse_grained_refs(ref_folder: pathlib.Path, squash_num: int, commit: str) -> list[Refactoring]:
    coarse_grained_path = ref_folder.joinpath(f"o{squash_num}")
    return load_dictref(coarse_grained_path.joinpath(commit + ".json"))


def get_retraced_commit_refdict2(squash_log_p: str, coarse_grained_commit, ignored_commit_count, sub_length) -> list[
    list[Refactoring]]:
    ...
    squash_num = pathlib.Path(squash_log_p).name.split(".txt")[0].split("log")[1]
    refs = []
    coarse_grained_path = refs_d.joinpath(str(sub_length), coarse_grained_commit + ".json")
    ...
    if ignored_commit_count == 0:
        ...
        refs_p = pathlib.Path(squash_log_p).parent.parent.joinpath("o" + str(sub_length)).joinpath(
            coarse_grained_commit + ".json")
...
def load_retraced_commit_refdict(ref_folder: pathlib.Path, coarse_grained_commit, ignored_commit_count,
                                 sub_length) -> list[
    list[Refactoring]]:
...
def extract_coarse_grained_refs_thorough_types(coarse_grained_refs: list[list[Refactoring]],
                                               normal_grained_refs: list[list[Refactoring]]) -> list[Refactoring]:
    """
    ICPC 2022
...
def match_oline_as_supportive(coarse_grained_ref: Refactoring, normal_grained_ref: Refactoring,
    ...
    if (coarse_grained_ref.type == normal_grained_ref.type and \
        coarse_grained_ref.refactored_source_location == normal_grained_ref.refactored_source_location) \
            or (coarse_grained_ref.type == normal_grained_ref.type and str(coarse_grained_ref.left) == str(
        normal_grained_ref.left) and str(coarse_grained_ref.right) == str(normal_grained_ref.right)):
        return True
    return False


def match_with_traced_location(refA: Refactoring, num_of_ignored_commit_A: int, refB: Refactoring,
                               num_of_ignored_commit_B: int, ) -> bool:
    """
    ...
    :return:
    """
    return refA.type == refB.type and refA.refactored_location.codeElement == refB.refactored_location.codeElement and \
        refA.traced_location[str(num_of_ignored_commit_A)] == refB.traced_location[
            str(num_of_ignored_commit_B)]
